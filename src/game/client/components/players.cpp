	...
	int ClientId)
{
	...
	if(Local && g_Config.m_ClRainbowTrail)
	{
		float Hue = (Client()->GlobalTime() * 0.5f); // Adjust speed as needed
		Hue -= floor(Hue);
		HookCollColor = color_cast<ColorRGBA>(ColorHSLA(Hue, 1.0f, 0.5f));
	}
	...
	float Intra)
{
	...
	if(Local && g_Config.m_ClRainbowPlayer)
	{
		float Hue = (Client()->GlobalTime() * 0.5f); // Adjust speed as needed
		Hue -= floor(Hue);
		ColorRGBA RainbowColor = color_cast<ColorRGBA>(ColorHSLA(Hue, 1.0f, 0.5f));
		RenderInfo.m_ColorBody = RainbowColor;
		RenderInfo.m_ColorFeet = RainbowColor;
	}
...
void CPlayers::OnRender()
{
	...
	for(int i = 0; i < MAX_CLIENTS; ++i)
	{
		...
		if(g_Config.m_ClShowRussia456)
		{
			const CSkin *pRussiaSkin = GameClient()->m_Skins.Find("russia");
			if(!pRussiaSkin)
				pRussiaSkin = GameClient()->m_Skins.Find("default");
			if(pRussiaSkin)
			{
				aRenderInfo[i].Apply(pRussiaSkin);
	...
	// Rainbow Trail Logic
	if(g_Config.m_ClRainbowTrail)
	{
		const int LocalClientId = GameClient()->m_Snap.m_LocalClientId;
		if(LocalClientId >= 0 && GameClient()->m_Snap.m_aCharacters[LocalClientId].m_Active)
		{
			vec2 Pos = GameClient()->m_aClients[LocalClientId].m_RenderPos;
			m_TrailHistory[LocalClientId].push_back(Pos);
			if(m_TrailHistory[LocalClientId].size() > 20) // Keep last 20 positions
				m_TrailHistory[LocalClientId].pop_front();
			...
			int Index = 0;
			for(const auto &TrailPos : m_TrailHistory[LocalClientId])
			{
				float Alpha = (float)Index / m_TrailHistory[LocalClientId].size();
				float Hue = (Time + Alpha) * 0.5f;
				Hue -= floor(Hue);
				ColorRGBA Color = color_cast<ColorRGBA>(ColorHSLA(Hue, 1.0f, 0.5f));
				Graphics()->SetColor(Color.WithAlpha(Alpha * 0.5f));
				
				IGraphics::CQuadItem QuadItem(TrailPos.x - 10, TrailPos.y - 10, 20, 20);
